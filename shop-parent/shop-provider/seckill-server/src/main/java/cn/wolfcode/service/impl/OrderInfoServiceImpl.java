...
//    private RocketMQTemplate rocketMQTemplate;
    ...
    @Transactional(rollbackFor = Exception.class) // 解决 this调用事务失效问题
    @Override
    public String doSeckill(Long seckillId, Long userPhone, Integer time) {
        ...
        return this.doSeckill(userInfo,seckillProductVo);
    }
    ...
    public void checkPyTimeout(OrderTimeoutMessage message) {
        ...
        if (row > 0) {
            ...
            // 4. 删除用户重复下单标识和回滚redis库存
            this.syncStock(message.getSeckillId(), message.getUserPhone());
            //5. 删除订单状态缓存
            String orderKey = SeckillRedisKey.SECKILL_ORDER_HASH.join(message.getUserPhone() + "");
            redisTemplate.opsForHash().delete(orderKey, message.getOrderNo()+"");
        }
    }
    ...
    public String alipay(String orderNo, String token) {
        // 1. 查询订单信息 只有未支付状态才能支付
        OrderInfo byOrderNo = this.findByOrderNo(orderNo);
        if (!byOrderNo.getStatus().equals(OrderInfo.STATUS_ARREARAGE)) {
            throw new BusinessException(SeckillCodeMsg.ORDER_STATUS_ERROR);
        }
        // 检查当前用户
        this.checkOrderUser(token,byOrderNo);

        //创建支付请求对象
        PayVo vo=new PayVo();
        vo.setOutTradeNo(orderNo);
        vo.setSubject(byOrderNo.getProductName());
        vo.setTotalAmount(byOrderNo.getSeckillPrice().toString());
        vo.setBody("秒杀商品"+byOrderNo.getProductName());

        // 远程调用支付服务发起预支付
        Result<String> result = alipayFeignApi.doPay(vo);
        if (result.hasError()){
...
//    }
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void alipaySuccess(PaySuccessVo vo) {
        // 基于订单号查询订单是否存在
        OrderInfo orderInfo = orderInfoMapper.find(vo.getOutTradeNo());
        if (orderInfo==null){
            throw new BusinessException(SeckillCodeMsg.REMOTE_DATA_ERROR);
        }
        // 判断支付金额是否正确
        if (!orderInfo.getSeckillPrice().equals(new BigDecimal(vo.getTotalAmount()))){
            throw new BusinessException(SeckillCodeMsg.PAY_AMOUNT_ERROR);
        }
        //基于乐观锁校验幂等性 订单状态只能修改一次
        int row = orderInfoMapper.changePayStatus(vo.getOutTradeNo(), OrderInfo.STATUS_ACCOUNT_PAID, OrderInfo.PAY_TYPE_ONLINE);
        if (row<=0){
            throw new BusinessException(SeckillCodeMsg.ORDER_STATUS_ERROR);
        }
        //删除订单状态缓存
        ...
        // 构建支付日志对象，保存支付日志
        PayLog log = this.buildPayLog(vo.getTradeNo(), orderInfo, PayLog.PAY_TYPE_ONLINE);
        try {
            payLogMapper.insert(log);
        } catch (SQLException e) {
    ...
    public void refund(String orderNo, String token) {
        ...
        RefundVo refundVo = new RefundVo(orderInfo.getOrderNo(),orderInfo.getSeckillPrice().toString(),"不想要了");
        Result<Boolean> result=null;
        if (orderInfo.getPayType()==OrderInfo.PAY_TYPE_ONLINE){
            // 远程调用支付服务 发起支付宝退款
             result = alipayFeignApi.refund(refundVo);
        }else {
            // 积分退款
        }
        ...
        this.syncStock(orderInfo.getSeckillId(),orderInfo.getUserId());
        // 修改订单状态  //保存退款日志 TODO 修改退款原因
        this.changeRefundStatus(orderInfo,"不想要了");
